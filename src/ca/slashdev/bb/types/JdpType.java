...
public class JdpType extends BaseJdpType {
   ...
   public JdpType(Node appDescriptor) {
      this();

      Node general = Utils.getChildNamed(appDescriptor, "General");
      parseCommonXml(general.getAttributes());

      NamedNodeMap genAttribs = general.getAttributes();
      version = Utils.getAttrString(genAttribs, "Version");
      vendor = Utils.getAttrString(genAttribs, "Vendor");
      description = Utils.getAttrString(genAttribs, "Description");

      Node app = Utils.getChildNamed(appDescriptor, "Application");
      parseCommonXml(app.getAttributes());

      NamedNodeMap appAttribs = app.getAttributes();
      type = TypeAttribute.fromAppDescriptor(Utils.getAttrString(appAttribs, "Type"));

      Node res = Utils.getChildNamed(appDescriptor, "Resources");
      parseCommonXml(res.getAttributes());

      parseIconsXml(Utils.getChildNamed(res, "Icons"));
   }
   ...
   public void writeManifest(File file, String output) throws BuildException {
      ...
      try {
         ...
         if (TypeAttribute.LIBRARY.equals(type.getValue())) {
            int flags = 0x02;
            ...
            if (TypeAttribute.CLDC.equals(type.getValue())) {
               ...
               printRibbonPosition(out, 1);
               printNameResource(out, 1);
               printIcons(out, 1);
               ...
               if (entryPoints.size() != 0) {
                  ...
                  for (EntryPointType entryPoint : entryPoints) {
                     if(entryPoint.enabled(getProject())) {
                        ...
                        entryPoint.printRibbonPosition(out, entryIndex);
                        entryPoint.printNameResource(out, entryIndex);
                        entryPoint.printIcons(out, entryIndex);
               ...
               if (entryPoints.size() != 0) {
                  ...
                  for (EntryPointType entryPoint : entryPoints) {
                     if(entryPoint.enabled(getProject())) {
                        out.printf("MIDlet-%d: %s,%s,%s\n", entryIndex, entryPoint.getTitle(),
                              entryPoint.getFirstIcon(), entryPoint.getMidletClass());
